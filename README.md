...
| Page content | The archive repeated the same six "How We Help You Win" tiles and the same closing line on almost every page. `build/detail.py` now gives every service, industry, company and support page its own substance (feature tiles, numbered process, proof points, comparison tables, service-page chips, a related case-study quote and page-specific FAQs), rendered by `build_detailed()` in `build/site.py`. Facts and figures come from the archived copy and case studies; the Our Work gallery and project pages use licensed photos per industry. |
| Imagery | The archive only hot-linked a CDN that no longer serves anything. Photography is now self-hosted under `build/assets/photos/` (41 Adobe Stock free-collection photos licensed to the site owner's Adobe account, served at 1600w/800w), chosen per industry and article topic by `build/illustrations.py`; the old app screenshots are SVG mockups, the portfolio uses industry photography, and the "trusted nationwide" section is a real Albers-projected map of the lower 48 (`build/usmap.py`, state boundaries from `build/data/us-states.json`) with one linked pin per market. No page depends on an external image or video. |
