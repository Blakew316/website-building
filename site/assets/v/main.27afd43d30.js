/* Charlie Company Media — site behaviour (no dependencies) */
(function () {
  'use strict';
  ...
  const fallbackSVG = (label) => {
    ...
    t = t.slice(0, 48);
    const caption = t ? '<text x="400" y="452" text-anchor="middle" font-family="-apple-system,Inter,Helvetica,Arial,sans-serif" font-size="22" font-weight="600" fill="#1e4b8f" opacity=".8">' + t.replace(/&/g, '&amp;') + '</text>' : '';
    return 'data:image/svg+xml;utf8,' + encodeURIComponent(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600"><defs><linearGradient id="g" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="#eef2f8"/><stop offset=".55" stop-color="#eef1f7"/><stop offset="1" stop-color="#eef1f6"/></linearGradient><linearGradient id="a" x1="0" x2="1"><stop offset="0" stop-color="#2a5a9c"/><stop offset="1" stop-color="#4f86c6"/></linearGradient></defs>'
  ...
  /* ---------- analytics (first-party, sent to /api/track) ---------- */
  const API = document.documentElement.dataset.api || '';
  const sid = (() => { try { let v = sessionStorage.getItem('ml_sid'); if (!v) { v = Math.random().toString(36).slice(2) + Date.now().toString(36); sessionStorage.setItem('ml_sid', v); } return v; } catch (e) { return 'anon'; } })();
  const vid = (() => { try { let v = localStorage.getItem('ml_vid'); if (!v) { v = Math.random().toString(36).slice(2) + Date.now().toString(36); localStorage.setItem('ml_vid', v); } return v; } catch (e) { return ''; } })();
  const utm = (() => { const u = new URLSearchParams(location.search), o = {}; ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid', 'fbclid'].forEach((k) => { if (u.get(k)) o[k] = u.get(k).slice(0, 120); }); return o; })();
  const track = (type, data) => {
    if (!API || location.protocol === 'file:') return;
    const payload = JSON.stringify({ type, path: location.pathname, title: document.title.slice(0, 160), ref: document.referrer.slice(0, 300), sid, vid, utm, sw: screen.width, sh: screen.height, lang: navigator.language, data: data || {} });
    try {
  ...
  window.mlTrack = track;
  track('pageview');
  // engagement: scroll depth + time on page, sent once when leaving
  let maxScroll = 0; const t0 = Date.now();
  const onDepth = () => { const h = document.documentElement; const d = Math.round((h.scrollTop + innerHeight) / h.scrollHeight * 100); if (d > maxScroll) maxScroll = Math.min(100, d); };
  window.addEventListener('scroll', onDepth, { passive: true }); onDepth();
  let left = false;
  const onLeave = () => { if (left) return; left = true; track('engagement', { scroll: maxScroll, seconds: Math.round((Date.now() - t0) / 1000) }); };
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') onLeave(); });
  window.addEventListener('pagehide', onLeave);
  // clicks: CTAs, phone, outbound
  document.addEventListener('click', (e) => {
    const a = e.target.closest('a, button'); if (!a) return;
    const href = a.getAttribute('href') || '';
    const label = (a.textContent || a.getAttribute('aria-label') || '').trim().slice(0, 80);
    if (href.startsWith('tel:')) track('call_click', { label, href });
    else if (a.classList.contains('btn') || a.classList.contains('btn-link') || a.classList.contains('tile-link')) track('cta_click', { label, href: href.slice(0, 200) });
    else if (/^https?:/.test(href) && !href.includes(location.host)) track('outbound', { href: href.slice(0, 200) });
  }, { passive: true });
  ...
  $$('form[data-form]').forEach((form) => {
    ...
    const formName = (form.querySelector('[name=form]') || {}).value || 'form';
    const show = (i) => {
    ...
    form.addEventListener('focusin', () => { if (!started) { started = true; track('form_start', { form: formName }); } }, { once: true });
    $$('[data-next]', form).forEach((b) => b.addEventListener('click', () => { if (validate($$('input,textarea,select', steps[idx]))) { show(Math.min(idx + 1, steps.length - 1)); track('form_step', { form: formName, step: idx + 1 }); } }));
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const fields = {}; data.forEach((v, k) => { if (k !== 'website') fields[k] = typeof v === 'string' ? v.slice(0, 2000) : ''; });
      let sent = false;
      // 1) built-in endpoint (Netlify function → Blobs → admin dashboard)
      if (API && location.protocol !== 'file:') {
        try {
          const res = await fetch(API + '/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ form: formName, page: location.pathname, ref: document.referrer.slice(0, 300), sid, vid, utm, fields }) });
          sent = res.ok;
      ...
      track('form_submit', { form: formName, ok: sent });
      try { localStorage.setItem('ml_lead_' + Date.now(), JSON.stringify(fields)); } catch (err) {}
      if (form.dataset.redirect && (sent || (!action && !API))) { window.location.href = form.dataset.redirect; return; }
      form.innerHTML = '<div class="form-success"><div class="icon-tile lg teal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5.5 12.5 9.75 16.75 18.5 8"/></svg></div><h3>Thanks — we\'ve got it.</h3></div>';
    });
  });
